...
if(BUILD_PYTHON)
  ...
  if(Python3_EXECUTABLE)
    ...
    if("${_exitcode}" EQUAL 0 AND IS_DIRECTORY "${_py_inc}")
      set(Python3_INCLUDE_DIR "${_py_inc}")
    ...
    if("${_exitcode}" EQUAL 0 AND IS_DIRECTORY "${_py_lib}")
      get_filename_component(_py_lib "${_py_lib}/../" ABSOLUTE)
      if(WIN32)
        set(_py_lib "${_py_lib}/libs")
      endif()
      find_library(Python3_LIBRARY NAMES python3 PATHS ${_py_lib} NO_DEFAULT_PATH)
      if(Python3_LIBRARY)
        message(STATUS "Setting Python library to ${Python3_LIBRARY} from sysconfig")
      endif()
    endif()
    pycmd_no_exit(_np_inc _exitcode "import numpy; print(numpy.get_include())")
    if("${_exitcode}" EQUAL 0 AND IS_DIRECTORY "${_np_inc}")
      set(Python3_NumPy_INCLUDE_DIR "${_np_inc}")
      message(STATUS "Setting Numpy's include dir to ${_np_inc} from sysconfig")
    endif()
  endif()
  ...
  if(Python3_FOUND)
    ...
    if(Python3_VERSION VERSION_LESS 3.7)
      message(FATAL_ERROR
        "Found Python libraries version ${Python3_VERSION}. Python <= 3.6 is no longer supported by PyTorch.")
    endif()

    add_library(python::python INTERFACE IMPORTED)
    target_include_directories(python::python INTERFACE ${Python3_INCLUDE_DIR})

    if(Python3_LIBRARY)
      target_link_libraries(python::python INTERFACE ${Python3_LIBRARY})
    endif()
    if(Python3_LIBRARIES)
      target_link_libraries(python::python INTERFACE ${Python3_LIBRARIES})
    endif()
    if(Python3_LIBRARY_DIRS)
      target_link_directories(python::python INTERFACE ${Python3_LIBRARY_DIRS})
    endif()
    if(_py_lib)
      target_link_directories(python::python INTERFACE ${_py_lib})
      if(MSVC)
        string(REPLACE "Lib" "libs" _py_static_lib ${_py_lib})
        message(STATUS "Use python link directory on Windows:${_py_static_lib}")
        link_libraries(${_py_static_lib})
      endif()
    endif()
    ...
    if(Python3_NumPy_INCLUDE_DIR)
      ...
      target_include_directories(numpy::numpy INTERFACE ${Python3_NumPy_INCLUDE_DIR})
    elseif(USE_NUMPY)
